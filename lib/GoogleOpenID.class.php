...
class GoogleOpenID
{
  ...
  static public function getLoginUrl()
  {
    ...
    $returnTo = sfContext::getInstance()->getRouting()->generate('sfGoogleLogin_verify', array(), true);

    return $googleOpenID->buildLoginUrl($returnTo);
  }
   ...
   * @return String URL
   */
  public function buildLoginUrl($returnTo)
  {
    $params = array(
  ...
  public function verifyLogin()
  {
    ...
    if ($params['openid_assoc_handle'] == $this->getHandle())
    {
      ...
      if ($this->debug)
        echo 'signatures did not match<br>';
      return false;
    }
    if ($this->debug)
      echo 'openid_assoc_handle did not match<br>';
    return false;
  }
